...
use crate::{desktop::Desktop, history};
...
impl App {
    ...
    fn select(&mut self) {
        ...
        if let Some(item) = item {
            ...
            let _ = history::write_to_file(&self.history)
    ...
    fn update_list(&mut self) {
        ...
        let mut items: Vec<_> = snapshot
            ...
            .enumerate()
            .map(|(index, item)| {
                let count = self.history.get(&item.data.name).copied().unwrap_or(0);
                let score = if snapshot.pattern().is_empty() {
                    count
                } else {
                    matched_count - index + (count * self.score_multiplier)
                };
                (item.data.name.as_str(), score, index)
            })
            .collect();

        items.sort_by_key(|(_, score, _)| cmp::Reverse(*score));
        self.list = items
            .iter()
            .take(max_count)
